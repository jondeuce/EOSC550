...
"""
function chi2d(;m=50,n=60,Ntrn=5,Ntst=2)
    # Initialize model
    fpi = Float32(pi)
    xp, yp = linspace(-fpi,fpi,m), linspace(-fpi,fpi,n)'

    _chi2d_ytrn = zeros(Float32,m,n,1,Ntrn)
    _chi2d_ytst = zeros(Float32,m,n,1,Ntst)
    ...
    _chi2d_xtst = _chi2d_ytst + 0.3randn(Float32,size(_chi2d_ytst));
    _chi2d_xtrn = _chi2d_ytrn + 0.3randn(Float32,size(_chi2d_ytrn));
